...
// =======================================================================
ini_set('display_errors', 0); // Matikan display error agar JSON response aman
...
// Jika koneksi gagal, stop script
if (!$db) {
...
// =======================================================================
// 2. AUTO-REPAIR DATABASE (EKSEKUSI LANGSUNG DI SINI)
// =======================================================================

// A. BUAT TABEL UTAMA: sim_inventory (Jika Belum Ada)
...
try {
    if ($db_type === 'pdo') {
        $db->exec($sql_create_inv);
    } else {
        mysqli_query($db, $sql_create_inv);
    }
} catch (Exception $e) {
    // Jika gagal buat tabel, hentikan proses agar error terlihat
    die("System Error: Gagal membuat tabel sim_inventory. " . $e->getMessage());
}

// B. PERBAIKI TABEL LAMA (Menambahkan kolom detail jika belum ada)
...
function readSpreadsheet($filePath, $originalName) {
    ...
    if ($ext === 'csv') {
        if (($handle = fopen($filePath, "r")) !== FALSE) {
            ...
            if(!empty(trim($firstLine))) $data[] = str_getcsv($firstLine);
...
// Helper: Find Column Index (Flexible)
function findIdx($headers, $keys) {
    foreach ($headers as $i => $v) { 
        // Bersihkan nama header: hapus spasi, titik, underscore, ubah ke lowercase
...
// =======================================================================
// 4. API HANDLERS (AJAX REQUESTS)
// =======================================================================

// --- API: FETCH SIMS FOR MODAL (BULK SEARCH) ---
if ($action == 'fetch_sims') {
    ...
    // Base Query
    $query = "SELECT id, msisdn, iccid, imsi, status FROM sim_inventory WHERE po_provider_id = ? AND status = ?";
    $params = [$po_id, $target_status];

    // Bulk Search Logic
    if (!empty($search_bulk)) {
        // Normalisasi input: ganti koma, spasi dengan newline, lalu explode
    ...
    $query .= " ORDER BY msisdn ASC LIMIT 500"; // Safety limit
    ...
    try {
        if ($db_type === 'pdo') {
            ...
            // Mysqli fallback (basic)
            $res = mysqli_query($db, "SELECT id, msisdn FROM sim_inventory WHERE po_provider_id='$po_id' AND status='$target_status' LIMIT 500");
...
if ($action == 'process_bulk_sim_action') {
    ...
    try {
        ...
        if ($db_type === 'pdo') {
            ...
            // 2. INSERT LOG SUMMARY (Agar Dashboard Chart tetap jalan)
            ...
            echo json_encode(['status'=>'success', 'count'=>$count, 'message'=>"$count SIMs successfully " . ($mode=='activate'?'Activated':'Terminated')]);
             ...
             echo json_encode(['status'=>'error', 'message'=>'PDO Driver Required for this feature']);
        }
    } catch (Exception $e) {
...
// =======================================================================
// 5. STANDARD HANDLERS (UPLOAD MASTER)
// =======================================================================
...
if ($action == 'upload_master_bulk') {
    try {
        ...
        if (isset($_FILES['upload_file']) && $_FILES['upload_file']['error'] == 0) {
            ...
            if (!$rows || count($rows) < 2) die("<script>alert('File Kosong atau Format Salah. Pastikan ada header MSISDN.');window.history.back();</script>");
            ...
            if ($idx_msisdn === false) die("<script>alert('Error: Kolom MSISDN tidak ditemukan di file Excel.');window.history.back();</script>");
...
// =======================================================================
// [LEGACY] 6. FITUR LAMA (PO, LOGISTIC, COMPANY, ETC)
// =======================================================================
...
if (isset($_POST['action']) && ($_POST['action'] === 'create' || $_POST['action'] === 'update')) {
    ...
    $id = $_POST['id'] ?? null;
    $type = $_POST['type'];
    // Validasi Integer (Fix bug strict mode)
...
// E. Legacy Activation (Manual Form Old) - kept for backward compatibility
